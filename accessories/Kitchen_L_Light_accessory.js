...
var MQTT_TOPIC = 'getAction_LightKL'; //MQTT topic that was set on the Sonoff firmware
var MQTT_TOPIC1 = 'Status_LightKL'; //MQTT topic that was set on the Sonoff firmware
var debug = false;
...
//var  status = 'on';
var LightController = {
  name: "L_Light", //name of accessory
  pincode: "031-45-154",
  username: "Z1:3C:ED:5A:1A:1A", // MAC like address used by HomeKit to differentiate accessories. 
  manufacturer: "HAP-NodeJS", //manufacturer (optional)
  model: "v1.0", //model (optional)
  serialNumber: "A12Z345KGB", //serial number (optional)
  ...
  getPower: function() { //get power of accessory
   ...
   // if (status == true) client.publish(MQTT_TOPIC, "On");
   // if (status == false) client.publish(MQTT_TOPIC, "OFF");
...
lightAccessory
  ...
  .on('get', function(callback) {
    ...
    client.on('message', function(topic, message) {
       ...
       Status_LightKL = parseFloat(message);
       if (Status_LightKL==1) {TURN=true;}
       if (Status_LightKL==0) {TURN=false;}
